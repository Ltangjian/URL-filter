 ...
 
{
    ...
    pf = fopen("/proc/rule_repository", "w");
 
    if (pf == NULL)  {
 
        printf("Cannot open /proc/rule_repository for writting\n");
...
void send_rule_to_proc()
{
    ...
    sprintf(a_rule, "%u %s %s %s %s %s %s %u %u %s\n", mf_rule.in_out+1, print_value(mf_rule.src_ip), print_value(mf_rule.src_netmask), print_value(mf_rule.src_port), print_value(mf_rule.dest_ip), print_value(mf_rule.dest_netmask), print_value(mf_rule.dest_port), get_proto(mf_rule.proto), get_action(mf_rule.action),mf_rule.key_word);
...
void print_rule()
{
    ...
    printf("in/out    src ip    src mask    src port    dest ip    dest mask     dest port    proto    action    key word\n");
 
    pf = fopen("/proc/rule_repository", "r");
 
    if (pf == NULL)  {
 
        printf("Cannot open /proc/rule_repository for reading\n");
...
int main(int argc, char **argv)
{
    ...
    mf_rule.action = NULL;mf_rule.key_word=NULL;
    while (1) 
    {
        ...
        int option_index = 0;
        c = getopt_long(argc, argv, "od:s:m:p:t:n:q:c:a:k:", long_options, &option_index);
