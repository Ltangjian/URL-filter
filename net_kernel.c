...
#define PROCF_NAME "rule_repository"
 
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Linux firewall");
MODULE_AUTHOR("Liu Tangjian");
...
/*Knuth-Morris-Pratt (KMP) String Search Algorithm source code*/
int kmp_search(const char *text, const char *pattern)
{
    int m=(strlen(pattern)+1) * sizeof(int);
    int T[m];
    int i, j;
    ...
    //T = (int*) kmalloc((strlen(pattern)+1) * sizeof(int) ,GFP_KERNEL);
    T[0] = -1;
    ...
    //printk(KERN_INFO "Finish KMP!!!!!"); 
    //printk(KERN_INFO "KMP result: %s!!!!!!!!!!!!!!!!!\n", result); 
    //kfree(T);
    if(result!=NULL){
      return 1;
    }else{
...
void add_a_rule(struct mf_rule_desp* a_rule_desp) {
    ...
    //change sizeof(*a_rule) to 200
    a_rule = kmalloc(200, GFP_KERNEL);

/*----------------------add by tanjon----------------------------
    a_rule->in_out = kmalloc(1, GFP_KERNEL);
    a_rule->src_ip = kmalloc(4, GFP_KERNEL);
    a_rule->src_netmask = kmalloc(4, GFP_KERNEL);
    a_rule->src_port = kmalloc(4, GFP_KERNEL);
    a_rule->dest_ip = kmalloc(4, GFP_KERNEL);
    a_rule->dest_netmask = kmalloc(4, GFP_KERNEL);
    a_rule->dest_port = kmalloc(4, GFP_KERNEL);
    a_rule->proto = kmalloc(1, GFP_KERNEL);
    a_rule->action = kmalloc(1, GFP_KERNEL);
    a_rule->key_word = (char *)kmalloc(16, GFP_KERNEL); */
    ...
    if (strcmp(a_rule_desp->key_word, "-") != 0){
        strcpy(a_rule->key_word,a_rule_desp->key_word);
        //printk(KERN_INFO "key_word=%s",a_rule_desp->key_word);
    }
        //a_rule->key_word=a_rule_desp->key_word;
    //else 
        //a_rule->key_word = NULL;
    a_rule->proto = a_rule_desp->proto;
    a_rule->action = a_rule_desp->action;
    printk(KERN_INFO "add_a_rule: in_out=%u, src_ip=%u, src_netmask=%u, src_port=%u, dest_ip=%u, dest_netmask=%u, dest_port=%u, proto=%u, action=%u, key_word=%s", a_rule->in_out, a_rule->src_ip, a_rule->src_netmask, a_rule->src_port, a_rule->dest_ip, a_rule->dest_netmask, a_rule->dest_port, a_rule->proto, a_rule->action, a_rule->key_word);
...
void init_mf_rule_desp(struct mf_rule_desp* a_rule_desp) {
    ...
    a_rule_desp->key_word = (char *)kmalloc(16, GFP_KERNEL); 
}
    ...
    if (offset > 0) {
        ...
        *eof = 1;
        ...
        list_for_each_entry(a_rule, &policy_list.list, list) {
            ...
            if (a_rule->key_word==NULL) {
 
		//printk(KERN_INFO "a_rule->key_word: %s!!!!!!!\n", a_rule->key_word);                
		strcpy(token, "-");
 
            } else {
 
                strcpy(token,a_rule->key_word);
...
//----------------------add by tanjon-------------------------------
   ...
   while (procf_buffer[i]!='\n') {
       ...
       rule_desp->key_word[j++] = procf_buffer[i++];
 
   }
 
   ++i;
 
   rule_desp->key_word[j] = '\0';
 
   printk(KERN_INFO "key_word: %s\n", rule_desp->key_word);
//-----------------------------------------------------------------
...
unsigned int hook_func_out(unsigned int hooknum, struct sk_buff *skb, 
   ...
   int num1,num2=0;
   //content = (char*) kmalloc(1500,GFP_KERNEL);
   for(num1=0;num1<skb->len;num1++){
        
        if(skb->data[num1]!='\0'){
	     
             content[num2]=skb->data[num1];
   ...
   list_for_each(p, &policy_list.list) {
       ...
       if (a_rule->in_out != 2) {
           ...
           if (a_rule->key_word == NULL) {
               ...
               //rule doens't specify key_word: match
 
           }
 
           else if (!kmp_search(content, a_rule->key_word)) {
 
               printk(KERN_INFO "rule %d not match: key_word mismatch\n", i);
